...
random.seed(2024)
np.random.seed(2024)
sample_skeleton_train_path = "./train/sample_skeleton_train.csv"
common_features_train_path = "./train/common_features_train.csv"
sample_skeleton_test_path = "./test/sample_skeleton_test.csv"
common_features_test_path = "./test/common_features_test.csv"
save_path = "./"
...
def preprocess_data(mode="train"):
    ...
    common_features_path = (
        common_features_train_path if mode == "train" else common_features_test_path
    )
    sample_skeleton_path = (
        sample_skeleton_train_path if mode == "train" else sample_skeleton_test_path
    )
    ...
    if mode == "train":
        ...
        for k, v in vocabulary.items():
            print(k + ":" + str(len(v)))
        new_vocabulary = dict(
            zip(sparse_columns, [[] for _ in range(len(sparse_columns))])
        )
        for k, v in vocabulary.items():
...
def reduce_mem(df):
    ...
    print(
        "-- Mem. usage decreased to {:5.2f} Mb ({:.1f}% reduction),time"
        " spend:{:2.2f} min".format(
            end_mem,
...
if __name__ == "__main__":
    ...
    train_data, val_data = train_test_split(
        train_data, test_size=0.2, random_state=2024
    )
    ...
    print(
        f"train_data : {len_train_data}, val_data: {len_val_data},"
        f" test_data:{len_test_data}"
    )
